...
def generate_graph(symbol, desc, data):
    ...
    if symbol in c.ANNOTATIONS:
        purchase_price, yield_since_purchase, yearly_yield = calc_yield_since_purchase(symbol, data)
        axs[0].annotate(c.ANNOTATIONS[symbol], xy=(data.index[-20], data['4. close'][-20]),
    ...
    if not three_month_data.empty:
        ...
        axs[1].set_title(f"{symbol} Stock Price (Last 3 Months)")
    ...
    plt.savefig(f"graph/{symbol}.png")
    plt.close()


def calc_yield_since_purchase(symbol, data):
    purchase_date = c.ANNOTATIONS[symbol].split(' on ')[-1]
    purchase_price = get_price_on_date(data, purchase_date)
    current_price = data['4. close'].iloc[-1]
    yield_since_purchase = calculate_yield(purchase_price, current_price) if purchase_price else None

    # Calculate yearly yield
    one_year_ago = datetime.now() - timedelta(days=365)
    one_year_data = data[data.index >= one_year_ago.strftime('%Y-%m-%d')]
    if not one_year_data.empty:
        price_one_year_ago = one_year_data['4. close'].iloc[0]
        yearly_yield = calculate_yield(price_one_year_ago, current_price)
    else:
        yearly_yield = None
...
def send_email_with_attachment():
    ...
    # Attach images
    for symbol in c.STOCKS_ETF:  # List of stocks
        with open(f"graph/{symbol}.png", "rb") as f:
            data = f.read()
